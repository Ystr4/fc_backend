...
namespace Stienen.Rabbit.Core.Tests {
    public class BasicBinaryMsgProviderTest {

        [Fact]
        public void Calls_Send_Test()
        {
            ChannelMock channel = new ChannelMock();
            BasicBinaryMsgProvider provider = new BasicBinaryMsgProvider(channel);

            ChangedDataDTO dto = new ChangedDataDTO(Guid.NewGuid(), Guid.NewGuid(), 158, 210, DateTime.Now, 0, new List<DataPart>(), StoreType.ChangedData);
            provider.Send(dto);
        }
    }
    ...
    public class ChannelMock : IModel {
