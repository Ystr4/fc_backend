...
//			Trace.TraceInformation("ctor {0}", ctor!=null);
		...
		//ASYNC
		public static IAsyncResult BeginExecProc(AsyncCallback callback, object state, string proc, params object[] args) {
			return BeginExecProc(proc, args, callback, state);
		}
		public static IAsyncResult BeginExecProc(string proc, object[] args, AsyncCallback callback, object state) {
			AsyncWrap<AsyncDbCommand> async=new AsyncWrap<AsyncDbCommand>(callback, state);
			IDbCommand cmd=DbCreateProc.CreateProc(DbConn.Open(), proc, args);
			//DbCreateProc.CreateReader(cmd); ??
			async.Object=new AsyncDbCommand(cmd);
			async.Wrapped=async.Object.BeginExecuteReader(CommandBehavior.CloseConnection|CommandBehavior.SequentialAccess, async.Complete, null);
			return async;
		}
		public static IEnumerable<object[]> EndExecProc(IAsyncResult result) {
			AsyncWrap<AsyncDbCommand> async=(AsyncWrap<AsyncDbCommand>)result;
			return Read(async.Object.EndExecuteReader(async.Wrapped));
		}
		public static void EndExecProc(Action<object[]> action, IAsyncResult result) {
			AsyncWrap<AsyncDbCommand> async=(AsyncWrap<AsyncDbCommand>)result;
			Read(async.Object.EndExecuteReader(async.Wrapped), action);
		}
		public static IEnumerable<T> EndExecProc<T>(IAsyncResult result) {
			AsyncWrap<AsyncDbCommand> async=(AsyncWrap<AsyncDbCommand>)result;
			return Read<T>(async.Object.EndExecuteReader(async.Wrapped));
		}
	}
}
